...
class Acceso{
	...
	function AccesoValida(){
		$query=mysql_query("SELECT a.id_usuario, CONCAT(a.nombre_usuario,' ',a.apellidos_usuario )AS user, r.nombre_rol, a.lectura_usuario AS lectura, a.escritura_usuario AS escritura, id_area
						FROM usuarios a, rol r
						WHERE a.id_rol=r.id_rol AND
						 a.login_usuario='".$_POST['usuario']."' AND
						 a.password_usuario='".$_POST['password']."' ") or die(mysql_error());
...
//acceso a las seciones da como resultado el menu que corresponde al usuario
	...
	function AccesoSecciones($id_usuario){
		$query=mysql_query("SELECT id_modulo, nombre_modulo FROM modulos ");
